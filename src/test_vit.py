...
def run_tests() -> None:
    ...
    # Exp2
    enc2 = ViTEncoder(image_size=224, patch_size=16, in_channels=3,
                      embed_dim=256, num_layers=4, num_heads=4)
    out2 = enc2(img)
    check("ViTEncoder Exp2 shape", out2.shape == (2, 196, 256))

    # Exp3
    enc3 = ViTEncoder(image_size=224, patch_size=16, in_channels=3,
    ...
    enc_drop = ViTEncoder(image_size=224, patch_size=16, in_channels=3,
                          embed_dim=256, num_layers=4, num_heads=4, dropout=0.5)
    enc_drop.train()
    with torch.no_grad():
        c = enc_drop(img)
        d = enc_drop(img)
    ...
    # ------------------------------------------------------------------
    print("\n--- Parameter count sanity ---")
    enc2_fresh = ViTEncoder(image_size=224, patch_size=16, in_channels=3,
                            embed_dim=256, num_layers=4, num_heads=4)
